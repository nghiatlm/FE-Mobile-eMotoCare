...
const SelectTimeStep = ({ state, dispatch, center }: any) => {
  ...
  const getSlot = async (centerId: string) => {
    try {
      ...
      console.log("getSlot response:", res);
      if (res?.success) {
        const data = res.data || {};
        ...
        if (Array.isArray(data?.slots)) s = data.slots;
        ...
        else if (Array.isArray(data?.serviceCente?.slot))
          s = data.serviceCente.slot;
          ...
          data?.servicecenter &&
          Array.isArray(data.servicecenter.serviceCente?.slot)
        )
          s = data.servicecenter.serviceCente.slot;
